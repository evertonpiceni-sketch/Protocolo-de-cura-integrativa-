...
type Props = { day: number; completed?: boolean; imageSrc: string; reducedMotion?: boolean };

export default function ChakraDailyMandala({ day, completed = false, imageSrc, reducedMotion = false }: Props) {
  ...
  return <figure className="text-center" aria-label={`Dia ${day}: ${chakra.name}, ${chakra.label}`}>
    <div className={`mx-auto aspect-square w-full max-w-[280px] rounded-full p-3 bg-black/5 ${completed ? 'shadow-xl' : 'opacity-90'}`}>
      <img src={imageSrc} alt={`Mandala tradicional de ${chakra.name} — ${chakra.label}`} className={`h-full w-full rounded-full object-contain ${reducedMotion ? '' : 'animate-[spin_28s_linear_infinite]'}`} />
    </div>
