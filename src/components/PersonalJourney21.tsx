  ...
  return <div className="ep-reintegration-shell relative min-h-screen bg-[#FAF7F2] px-4 py-5 text-[#2A2421]">
    ...
    <main className="mx-auto w-full max-w-[620px]">
      ...
      <section className="ep-reintegration-panel overflow-hidden rounded-[2rem]">
        <div className="ep-reintegration-luminous px-6 py-7 text-[#2A2421] bg-gradient-to-br from-[#FBF7F1] to-[#F3E8D8]">
          ...
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-[#E8E0D5]"><div className="h-full rounded-full bg-[#C89D52]" style={{width:`${Math.round((completed.length/21)*100)}%`}}/></div><p className="mt-2 text-xs text-[#7D736A]">{completed.length} de 21 momentos concluídos</p>
        </div>
        <div className="space-y-5 p-5 text-[#2A2421] sm:p-6">
          <div className="grid grid-cols-7 gap-2">{REINTEGRATION_DAYS.map(entry => <button key={entry.day} onClick={() => setDay(entry.day)} aria-label={`Abrir dia ${entry.day}`} className={`flex aspect-square items-center justify-center rounded-full border text-xs font-bold shadow-[0_8px_18px_rgba(0,0,0,.12)] ${day===entry.day?'border-[#efd78f] bg-gradient-to-br from-[#d9b85f] to-[#a8782a] text-[#173f35]':completed.includes(entry.day)?'border-[#C89D52]/55 bg-[#C89D52]/14 text-[#efd78f]':'border-[#C89D52]/18 bg-white/80 text-[#7D736A]'}`}>{completed.includes(entry.day)?<Check size={14}/>:entry.day}</button>)}</div>
          <article className="ep-reintegration-card rounded-3xl border border-[#E8E0D5] bg-white/85 p-5 shadow-sm"><p className="text-xs uppercase tracking-[.16em] text-[#C89D52]">Intenção do dia</p><p className="mt-2 text-base leading-7 text-[#5F554E]">{item.intention}</p></article>
          <section className="relative overflow-hidden rounded-[2rem] border border-[#E8E0D5] bg-white/80 shadow-[0_18px_44px_rgba(90,72,52,.08)]">
            <div className="relative flex h-[500px] flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-[#FFFDFC] to-[#F8F2E9] px-6">
              ...
              <div className="relative z-10 flex w-full flex-col items-center">
                ...
                <div className="w-full max-w-sm rounded-2xl border border-[#E8E0D5] bg-white/85 p-4 text-center shadow-sm backdrop-blur-md">
                  ...
                  <p className="mt-2 text-sm text-[#7D736A]">{playing?'Respire. Permaneça aqui. A luz acompanha o seu momento.':'Voz feminina, música e uma experiência visual de presença.'}</p>
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#E8E0D5]"><div className="h-full rounded-full bg-gradient-to-r from-[#E2B96B] to-[#C49746] transition-all" style={{width:`${audioProgress}%`}}/></div>
                </div>
              </div>
            </div></section>
          <label className="ep-reintegration-card flex cursor-pointer items-start gap-3 rounded-3xl border border-[#E8E0D5] bg-white/85 p-5 shadow-sm"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)} className="mt-1 h-5 w-5 accent-[#d6ae52]"/><span className="text-sm leading-6 text-[#3E352F]">{getReintegrationAcceptance(day)}</span></label>
          <button disabled={!accepted} onClick={startGuidedMeditation} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-[#E2B96B] to-[#C49746] px-5 py-4 font-bold text-[#1F1B18] shadow-md shadow-[#C89D52]/20 disabled:opacity-40">{playing?<Pause size={20}/>:started?<Play size={20}/>:<Headphones size={20}/>} {playing?'Pausar meditação':started?'Continuar meditação':'Iniciar meditação guiada'}</button>
          <article className="ep-reintegration-card rounded-3xl border border-[#E8E0D5] bg-white/85 p-5 shadow-sm">
            <p className="text-xs uppercase tracking-[.16em] text-[#C89D52]">Energias trabalhadas nesta etapa</p>
            <p className="mt-2 text-sm leading-6 text-[#7D736A]">Neste dia, as práticas energéticas foram organizadas para acompanhar a intenção “{item.title}”. A proposta é oferecer um campo de presença e integração enquanto você respira, observa o corpo e percorre a meditação. As ativações são programadas por Everton e você permanece livre para interromper a experiência a qualquer momento.</p>
            <div className="mt-4 space-y-3">{item.energyNotes.map((note,index)=><div key={note.name} className="rounded-2xl border border-[#C89D52]/14 bg-white/70 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.02)]"><strong className="block text-sm text-[#2A2421]">{note.name}</strong><span className="mt-2 block text-sm leading-6 text-[#7D736A]"><b>Foco nesta prática:</b> {note.focus}</span><span className="mt-2 block text-sm leading-6 text-[#7D736A]">{index===0?`Durante a condução, esta combinação é utilizada como apoio energético ao propósito de ${item.title.toLowerCase()}, acompanhando a respiração, a presença corporal e o movimento interno proposto para o Dia ${day}.`:`As frequências associadas a estes elementos acompanham as regiões e qualidades indicadas acima, funcionando dentro da proposta espiritual desta jornada como suporte à integração do trabalho do dia.`}</span></div>)}</div>
          </article>
          <p className="flex items-start gap-2 text-xs leading-5 text-[#8C8278]"><ShieldCheck size={17} className="mt-0.5 shrink-0 text-[#d6ae52]"/>Esta jornada é uma experiência espiritual e integrativa complementar. Ela não substitui atendimento médico, psicológico ou apoio humano necessário.</p>
          <div className="flex items-center justify-between gap-3 border-t border-[#C89D52]/16 pt-5"><button disabled={day===1} onClick={()=>setDay(day-1)} className="flex items-center gap-1 rounded-full border border-[#C89D52]/18 bg-white/75 px-3 py-2 text-sm font-semibold text-[#3E352F] disabled:opacity-30"><ChevronLeft size={18}/>Anterior</button><button onClick={complete} className="ep-gold-button rounded-full px-5 py-3 text-sm font-bold">{day===21?'Concluir jornada':'Concluir dia'}</button><button disabled={day===21} onClick={()=>setDay(day+1)} className="flex items-center gap-1 rounded-full border border-[#C89D52]/18 bg-white/75 px-3 py-2 text-sm font-semibold text-[#3E352F] disabled:opacity-30">Próximo<ChevronRight size={18}/></button></div>
        </div>
      </section>
    </main>
    {started && <section className="fixed inset-0 z-[120] flex min-h-[100dvh] flex-col overflow-hidden bg-[#FAF7F2] text-[#2A2421]" aria-label="Meditação guiada em andamento">
      ...
      <div className="relative z-10 space-y-4 bg-gradient-to-t from-[#FAF7F2] via-[#FAF7F2]/96 to-transparent px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-12 text-center">
        <p className="text-xs font-semibold uppercase tracking-[.22em] text-[#C89D52]">Dia {day} · {item.title}</p>
        <p className="font-display text-2xl text-[#2A2421]">{playing?(elapsedSeconds>=1260&&elapsedSeconds<1620?'Absorção e silêncio':'Permaneça neste momento'):audioProgress>=100?'Meditação concluída':'Meditação pausada'}</p>
        <p className="mx-auto max-w-md text-base leading-7 text-[#5F554E]">{reflectionText}</p>
        <p className="text-xs text-[#7D736A]">{elapsedSeconds>=1260&&elapsedSeconds<1620?'Apenas a música permanece enquanto você integra a experiência.':'Permaneça com esta reflexão até a próxima condução da voz.'}</p>
        <div className="mx-auto max-w-md"><div className="h-2 overflow-hidden rounded-full bg-[#E8E0D5]"><div className="h-full rounded-full bg-gradient-to-r from-[#E2B96B] to-[#C49746] transition-all duration-500" style={{width:`${audioProgress}%`}}/></div><div className="mt-2 flex justify-between text-xs text-[#8C8278]"><span>{formatTime(elapsedSeconds)}</span><span>{formatTime(totalSeconds)}</span></div></div>
        <div className="flex items-center justify-center gap-3"><button onClick={rewindMeditation} className="flex items-center justify-center gap-2 rounded-full border border-[#E0D4C3] bg-white/80 px-4 py-4 text-sm font-semibold text-[#524842]" aria-label="Voltar 15 segundos"><RotateCcw size={20}/>15s</button><button onClick={startGuidedMeditation} className="flex min-w-48 items-center justify-center gap-3 rounded-full bg-gradient-to-r from-[#E2B96B] to-[#C49746] px-6 py-4 font-bold text-[#1F1B18] shadow-md shadow-[#C89D52]/20">{playing?<Pause size={21}/>:<Play size={21}/>} {playing?'Pausar':audioProgress>=100?'Ouvir novamente':'Continuar'}</button></div>
      </div>
    </section>}
  </div>;
}
