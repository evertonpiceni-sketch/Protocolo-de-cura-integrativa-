        ...
        <div className="space-y-5 p-5 sm:p-6">
          ...
          <section className="relative overflow-hidden rounded-[2rem] border border-[#d6ae52]/40 bg-[#173f35] shadow-inner">
            <div className="relative h-[500px] overflow-hidden">
              <img src="/brand/human-chakra-model.jpg" alt="Corpo inteiro em meditação com os sete chakras" className={`absolute inset-0 h-full w-full object-cover object-top transition-all duration-1000 ${playing ? 'brightness-110 saturate-125' : 'brightness-50 saturate-60'}`}/>
              <div className="absolute inset-0 bg-gradient-to-t from-[#173f35] via-transparent to-[#173f35]/25"/>
              <div className={`absolute left-1/2 top-[9%] h-28 w-28 -translate-x-1/2 rounded-full bg-[#ffe39a]/70 blur-2xl transition-all duration-1000 ${playing ? 'scale-150 animate-pulse opacity-100' : 'scale-75 opacity-25'}`} />
              {[
                ['#a855f7','15%'],['#6366f1','23%'],['#38bdf8','31%'],['#22c55e','42%'],['#eab308','52%'],['#f97316','61%'],['#ef4444','70%']
              ].map(([color, top], index) => <span key={color} className={`absolute left-1/2 h-5 w-5 -translate-x-1/2 rounded-full border border-white/70 transition-all duration-700 ${playing ? 'animate-pulse opacity-100' : 'opacity-35'}`} style={{top, backgroundColor: color, boxShadow: playing ? `0 0 ${18 + index * 2}px 8px ${color}` : `0 0 8px 2px ${color}`}} />)}
              <div className={`absolute left-1/2 top-[43%] h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#f4d46e]/55 transition-all duration-1000 ${playing ? 'animate-ping opacity-55' : 'opacity-10'}`}/>
