...
export default function PersonalJourney21({ onClose }: Props) {
  ...
  return <div className="min-h-screen bg-[#9fbaa2] px-4 py-5 text-[#173f35]">
    ...
    {started && <section className="fixed inset-0 z-[120] flex min-h-[100dvh] flex-col overflow-hidden bg-[#061b14] text-[#fffaf0]" aria-label="Meditação guiada em andamento">
      <img src="/brand/human-chakra-model.jpg" alt="Corpo em meditação com os sete chakras" className="absolute inset-0 h-full w-full object-cover object-center transition-all duration-[3000ms]" style={{filter:`brightness(${.58+visualLife*.52}) saturate(${.72+visualLife*.5})`,transform:`scale(${1.01+visualLife*.055})`}}/><div className="absolute inset-0 bg-gradient-to-b from-[#03130d]/75 via-transparent to-[#03130d]/95"/><div className={`pointer-events-none absolute left-1/2 top-[8%] h-40 w-40 -translate-x-1/2 rounded-full bg-[#ffe39a]/70 blur-3xl transition-all duration-[3000ms] ${playing?'animate-pulse':''}`} style={{opacity:.18+visualLife*.78,transform:`translateX(-50%) scale(${.7+visualLife*1.05})`}}/>
