...
export default function PersonalJourney21({ onClose }: Props) {
  ...
  const startGuidedMeditation = async () => {
    ...
    if (musicRef.current) {
      ...
      await musicRef.current.play().catch(() => setAudioError(true));
    }
    await audioEngine.playGuidedMeditation([item.meditation], {
      title: `Dia ${day} — ${item.title}`,
      subtitle: '21 Dias para Voltar para Mim · Reintegração da Vida',
      voiceId: 'Rachel',
      volume: 1,
      stability: 0.52,
      similarityBoost: 0.78,
      onStart: () => setPlaying(true),
      onTimeUpdate: (seconds, duration) => duration > 0 && setAudioProgress(Math.min(100, (seconds / duration) * 100)),
      onEnd: () => {
        musicRef.current?.pause();
        setPlaying(false);
    ...
    <main className="mx-auto w-full max-w-[620px]">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-[#d6ae52]/40 bg-[#fffaf0]/95 shadow-[0_24px_70px_rgba(42,77,58,.18)]">
        ...
        <div className="space-y-5 p-5 sm:p-6">
          ...
          <section className="relative overflow-hidden rounded-[2rem] border border-[#d6ae52]/40 bg-[#dfe9dc] shadow-inner">
            <div className={`absolute left-1/2 top-[12%] h-28 w-28 -translate-x-1/2 rounded-full bg-[#ffe39a]/70 blur-2xl transition-all duration-1000 ${playing ? 'scale-150 opacity-100' : 'scale-75 opacity-35'}`} />
            <div className={`absolute left-1/2 top-1/2 h-52 w-52 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#d6ae52]/40 transition-all duration-1000 ${playing ? 'animate-ping opacity-50' : 'opacity-15'}`} />
            <div className="relative h-[390px] overflow-hidden">
              <img src="/brand/forest-app-background.png" alt="Natureza iluminada" className="absolute inset-0 h-full w-full object-cover opacity-55"/>
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_18%,rgba(255,225,139,.62),transparent_33%),linear-gradient(to_top,#e8efe3_0%,transparent_58%)]"/>
              <img src="/brand/chakra-body.png" alt="Corpo em meditação com os chakras" className={`absolute bottom-0 left-1/2 h-[91%] -translate-x-1/2 object-contain drop-shadow-xl transition-all duration-1000 ${playing ? 'scale-[1.03] brightness-110 saturate-125' : 'brightness-90 saturate-75'}`}/>
              <div className={`absolute inset-x-[18%] bottom-[17%] h-20 rounded-full bg-[#f4d46e]/30 blur-2xl transition-opacity duration-1000 ${playing ? 'animate-pulse opacity-100' : 'opacity-25'}`}/>
              <div className="absolute inset-x-4 bottom-4 rounded-2xl border border-white/60 bg-white/72 p-4 text-center backdrop-blur-md">
                <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#9a742b]">{playing ? 'Meditação guiada em andamento' : 'Sua meditação guiada'}</p>
                <p className="mt-2 text-sm text-[#587d67]">{playing ? 'Respire. Permaneça aqui. A luz acompanha o seu momento.' : 'Voz feminina, música e uma experiência visual de presença.'}</p>
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#d8e2d6]"><div className="h-full rounded-full bg-[#d6ae52] transition-all" style={{width:`${audioProgress}%`}}/></div>
              </div>
            </div>
          </section>
          ...
          {audioError && <p role="alert" className="rounded-2xl border border-[#b56f55]/30 bg-[#fff3ed] p-4 text-sm leading-6 text-[#8a4b37]">A voz guiada não pôde ser carregada agora. Tente novamente em alguns instantes.</p>}
