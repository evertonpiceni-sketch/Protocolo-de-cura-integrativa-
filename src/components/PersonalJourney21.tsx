...
export default function PersonalJourney21({ onClose }: Props) {
  ...
  return <div className="ep-reintegration-shell min-h-screen px-4 py-5 text-[#fff8e7]" style={{background:`radial-gradient(circle at 50% 0,rgba(245,213,127,${0.08 + journeyLight * 0.22}),transparent 34rem),linear-gradient(180deg,${shellTop},${shellBottom})`}}>
    ...
    <main className="mx-auto w-full max-w-[620px]">
      <button onClick={() => { stopSession(); onClose(); }} className="mb-4 flex items-center gap-2 rounded-full border border-[#d6ae52]/25 bg-[#031b13]/72 px-4 py-2 text-sm font-semibold text-[#f2e7c6] shadow-[0_10px_24px_rgba(0,0,0,.18)] backdrop-blur-md"><ArrowLeft size={17}/>Voltar ao início</button>
      <section className="ep-reintegration-panel overflow-hidden rounded-[2rem]">
        <div className="ep-reintegration-luminous px-6 py-7 text-[#fffaf0]" style={{background:`linear-gradient(135deg,rgba(5,42,30,${0.98 - journeyLight * 0.16}),rgba(15,74,49,${0.9 - journeyLight * 0.12}))`}}>
          <p className="text-xs font-semibold uppercase tracking-[.2em] text-[#ead28d]">21 Dias para Voltar para Mim</p><p className="mt-2 font-display text-xl italic">Reintegração da Vida</p><h1 className="mt-3 font-display text-3xl leading-tight">Dia {day}: {item.title}</h1><p className="mt-3 text-base leading-7 text-[#e7eee5]">{item.cycle}</p>
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-black/20"><div className="h-full rounded-full bg-[#d6ae52]" style={{width:`${Math.round((completed.length/21)*100)}%`}}/></div><p className="mt-2 text-xs text-[#dce8d8]">{completed.length} de 21 momentos concluídos</p>
        </div>
        <div className="space-y-5 p-5 text-[#fff8e7] sm:p-6">
          ...
          <article className="ep-reintegration-card rounded-3xl p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-[#b9cdbf]">Neste dia, as práticas energéticas foram organizadas para acompanhar a intenção “{item.title}”. A proposta é oferecer um campo de presença e integração enquanto você respira, observa o corpo e percorre a meditação. As ativações são programadas por Everton e você permanece livre para interromper a experiência a qualquer momento.</p>
            <div className="mt-4 space-y-3">{item.energyNotes.map((note,index)=><div key={note.name} className="rounded-2xl border border-[#d6ae52]/14 bg-[#031b13]/48 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.02)]"><strong className="block text-sm text-[#fff8e7]">{note.name}</strong><span className="mt-2 block text-sm leading-6 text-[#b9cdbf]"><b>Foco nesta prática:</b> {note.focus}</span><span className="mt-2 block text-sm leading-6 text-[#b9cdbf]">{index===0?`Durante a condução, esta combinação é utilizada como apoio energético ao propósito de ${item.title.toLowerCase()}, acompanhando a respiração, a presença corporal e o movimento interno proposto para o Dia ${day}.`:`As frequências associadas a estes elementos acompanham as regiões e qualidades indicadas acima, funcionando dentro da proposta espiritual desta jornada como suporte à integração do trabalho do dia.`}</span></div>)}</div>
          </article>
          <p className="flex items-start gap-2 text-xs leading-5 text-[#9fb7a8]"><ShieldCheck size={17} className="mt-0.5 shrink-0 text-[#d6ae52]"/>Esta jornada é uma experiência espiritual e integrativa complementar. Ela não substitui atendimento médico, psicológico ou apoio humano necessário.</p>
          <div className="flex items-center justify-between gap-3 border-t border-[#d6ae52]/16 pt-5"><button disabled={day===1} onClick={()=>setDay(day-1)} className="flex items-center gap-1 rounded-full border border-[#d6ae52]/18 bg-[#073426]/65 px-3 py-2 text-sm font-semibold text-[#d7e4db] disabled:opacity-30"><ChevronLeft size={18}/>Anterior</button><button onClick={complete} className="ep-gold-button rounded-full px-5 py-3 text-sm font-bold">{day===21?'Concluir jornada':'Concluir dia'}</button><button disabled={day===21} onClick={()=>setDay(day+1)} className="flex items-center gap-1 rounded-full border border-[#d6ae52]/18 bg-[#073426]/65 px-3 py-2 text-sm font-semibold text-[#d7e4db] disabled:opacity-30">Próximo<ChevronRight size={18}/></button></div>
        </div>
      </section>
    </main>
