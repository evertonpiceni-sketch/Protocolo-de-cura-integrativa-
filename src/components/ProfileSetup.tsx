...
export default function ProfileSetup({ onComplete }: ProfileSetupProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => onComplete(account), 700);
    } catch {
        ...
        {activeTab === 'register' ? (
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <label className="space-y-1.5"><span className="text-xs text-slate-400">Nome completo *</span><div className="relative"><User size={16} className="absolute left-3 top-3.5 text-slate-500" /><input value={fullName} onChange={e => setFullName(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3 pl-9" placeholder="Seu nome" /></div></label>
              <label className="space-y-1.5"><span className="text-xs text-slate-400">E-mail *</span><div className="relative"><Mail size={16} className="absolute left-3 top-3.5 text-slate-500" /><input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3 pl-9" placeholder="voce@email.com" /></div></label>
              <label className="space-y-1.5"><span className="text-xs text-slate-400">Data de nascimento *</span><input type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3" /></label>
              ...
              <label className="space-y-1.5"><span className="text-xs text-slate-400">WhatsApp</span><input value={phone} onChange={e => setPhone(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3" placeholder="(00) 00000-0000" /></label>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <label className="space-y-1.5"><span className="text-xs text-slate-400">Crie seu login *</span><input value={regLogin} onChange={e => setRegLogin(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3" placeholder="seu_login" /></label>
              <label className="space-y-1.5"><span className="text-xs text-slate-400">Crie sua senha *</span><div className="relative"><Lock size={16} className="absolute left-3 top-3.5 text-slate-500" /><input type="password" value={regPassword} onChange={e => setRegPassword(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3 pl-9" placeholder="mínimo 6 caracteres" /></div></label>
            </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <label className="space-y-1.5 block"><span className="text-xs text-slate-400">Login</span><input value={login} onChange={e => setLogin(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3" placeholder="seu_login" /></label>
            <label className="space-y-1.5 block"><span className="text-xs text-slate-400">Senha</span><input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full rounded-xl bg-slate-950 border border-slate-700 p-3" placeholder="Sua senha" /></label>
