...
export function createApp() {
  ...
  app.use(express.json({ limit: "1mb" })); app.use(cookieParser()); app.use('/api/', globalLimiter);

  const authenticate = (req: any, res: any, next: any) => { const token = req.cookies.token; if (!token) return res.status(401).json({ error: "Não autorizado." }); try { const decoded = jwt.verify(token, JWT_SECRET) as any; req.userId = decoded.userId; const db = getDb(); const user = db.users.find(u => u.id === req.userId); if (!user) { res.clearCookie("token"); return res.status(401).json({ error: "Usuário não encontrado." }); } req.user = user; next(); } catch { res.status(401).json({ error: "Sessão inválida ou expirada." }); } };
  ...
  app.post("/api/user/sync", authenticate, async (req: any, res: any) => { const db = getDb(); const userIndex = db.users.findIndex(u => u.id === req.userId); if (userIndex === -1) return res.status(404).json({ error: "Usuário não encontrado." }); const { profile, progress } = req.body; if (profile) db.users[userIndex].profile = { ...db.users[userIndex].profile, ...profile, plan: db.users[userIndex].plan, subscriptionPlan: db.users[userIndex].profile.subscriptionPlan, subscriptionExpiresAt: db.users[userIndex].profile.subscriptionExpiresAt, proActiveSince: db.users[userIndex].profile.proActiveSince }; if (progress) db.users[userIndex].progress = progress; try { await saveDb(); return res.json({ success: true }); } catch (err) { console.error("Error syncing user:", err); return res.status(503).json({ error: "Não foi possível salvar o perfil agora.", code: "PERSISTENCE_UNAVAILABLE" }); } });
  app.post("/api/payment/create-checkout", authenticate, apiLimiter, (_req: any, res: any) => res.status(503).json({ error: "Pagamento ainda não está configurado." }));
  app.post("/api/webhooks/payment", express.raw({ type: "application/json" }), (_req: any, res: any) => res.status(503).json({ error: "Webhook de pagamento ainda não está configurado." }));
