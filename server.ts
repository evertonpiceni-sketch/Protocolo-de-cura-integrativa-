...
export function createApp() {
  ...
  app.post("/api/user/sync", authenticate, (req: any, res: any) => {
    const db = getDb();
    ...
    if (progress) {
      db.users[userIndex].progress = progress;
    }
  ...
  async function handleProcessAnamnese(req: any, res: any) {
    try {
      ...
      const { anamnesis, language = "pt" } = req.body;
  ...
  app.get("/api/elevenlabs/voices", authenticate, apiLimiter, async (req, res) => {
    ...
    if (!client) {
      return res.json({ voices: defaultCuratedVoices, isCustomApiKey: false });
    }
